...
>
> 内容整理自个人学习笔记，并参考《大型网站技术架构：核心原理与案例分析》（李智慧）。分表后的路由方案见 [分表与路由.md](../数据存储/mysql/分表与路由.md)。
...
单机时代 `AUTO_INCREMENT` 够用，一旦**分库分表**（路由方案见 [分表与路由.md](../数据存储/mysql/分表与路由.md)）就失效了：
...
- [../数据存储/mysql/分表与路由.md](../数据存储/mysql/分表与路由.md) — 分片键与 ID 生成必须一起设计
- [../数据存储/mysql/数据类型选型.md](../数据存储/mysql/数据类型选型.md) — bigint 与时间列的口径
