...
## 二、微服务的注册、发现与健康检查是怎么实现的？（以 etcd 为例）
...
**说明**：第一节讲了"有哪些方案"，本题**补上 etcd 方案的实现细节**——
...
- **和 K8s 的关系？** → K8s 内部**也是用 etcd 存服务与端点信息**，
  ...
  相当于把这套机制**做成了平台能力**——所以用了 K8s 就不用自己实现（见第一节的方案 B）。
...
- **健康检查是主动还是被动？** → etcd 方案是**被动式**：
  **不额外起探测器**，靠"心跳停 → 租约到期 → key 消失"自然实现（详见本册第二节）。
...
### 6. 四种注册中心的"健康检查语义"对照（正文第二节/第三节的收口）
