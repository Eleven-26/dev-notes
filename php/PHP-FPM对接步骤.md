...
- **为什么 `php-fpm` 必须是 PID 1？** → 只有 1 号进程才能收到 K8s 发来的 SIGTERM。如果前面套了 shell，信号收不到，容器只能等宽限期到点被强杀，**最后一批上报数据就丢了**；`exec` 就是为这一步服务的。
- **`${GROUP_NAME}` 这类占位符没被替换会怎样？** → 扩展会拿着字面量当服务名上报，链路里会冒出一批畸形服务名；验证方式是在容器里 `grep` 实际生效的配置文件。
- **`/dev/shm` 为什么要单独挂？** → 探针用共享内存做本地缓冲，容器默认只有 64MB，写满会导致上报静默丢失；容量要按「并发 × 单请求缓冲 × 安全系数」估算。
- **前台启动 `-F` 不加会怎样？** → php-fpm 默认 daemonize，主进程 fork 完就退出 → 容器认为主进程已结束 → **容器秒退**，日志里看不到任何业务报错。
- **怎么保证停机时不丢数据？** → 靠整条优雅停机链路：SIGTERM → stop signal 转发 → 排空共享内存缓冲 → 进程退出；任何一环断了，表现都是「重启后指标缺口」。
