...
> Nacos 的定位、核心概念与架构、部署，以及 Java / Go 两套「配置管理 + 服务注册发现」的可落地用法与生产避坑。
>
> 内容整理自个人学习笔记；**敏感配置加密**一节按 photography-server 生产项目的实际实现（字段级 `ENCv1:` 密文 + AES-256-GCM + 密钥外置）整理。
> 服务发现与负载均衡的通用原理见 [服务发现与负载均衡.md](../分布式/服务发现与负载均衡.md)。
...
## 七、与 Spring Cloud / Kratos / K8s 的关系
...
**考察意图**：配置中心最容易出的安全问题是"**把 Nacos 当配置文件仓库用**"——数据库密码、AK/SK、第三方 token 明文躺在配置里。
一旦控制台账号泄露或配置被导出，等于**整套生产凭证外泄**。这一节给出一套可直接落地的方案（AES-256-GCM 字段级加密 + 密钥外置），
也是生产项目里的实际做法。
...
| 风险 | 后果 |
|---|---|
| **Nacos 控制台是 Web 系统** | 任何有读权限的人（含交接未完成的账号）都能直接看到明文 |
| **配置会被导出 / 备份** | 导出文件流转到哪儿不可控，等于凭证随配置文件扩散 |
| **环境之间复制配置** | 把 prod 配置复制到 test 时，明文凭证被一起带过去 |

> ⭐ 关键认知：**配置中心不是保险箱，它只是"分发通道"** —— 敏感值应以**密文**形式存放，解密能力留在**应用侧**。

### 9.2 三种方案对比

| 方案 | 做法 | 优点 | 缺点 |
|---|---|---|---|
| **A. 整库 / 整通道加密** | 用 Nacos 加解密插件，或对所有配置统一加密 | 一把梭、改动小 | **密钥在服务端**、粒度粗、改一个值要动全量 |
| **B. 字段级 ENC 密文**（本文推荐） | 只把敏感字段加密成 `ENCv1:...`，与明文混布 | **粒度细、可混布**、**KEK 不进配置中心** | 需要自建一条工具链（加密 / 校验） |
| **C. 完全外置** | 敏感值全放 K8s Secret / Vault，配置里只留引用 | 密钥与配置彻底分离 | 需额外组件，本地开发不便 |

### 9.3 密文格式与算法

```text
ENCv1:<base64( nonce(12B) | ciphertext | tag(16B) )>
```

| 选择 | 理由 |
|---|---|
| **AES-256-GCM** | 自带**完整性校验**（tag）：密文被篡改会**解密失败**，而不是解出一段垃圾 |
| **随机 nonce（12 字节）** | 每次加密结果都不同 —— **同一明文两次加密得到的密文不一样**，避免"看到相同密文推断出相同密码" |
| **版本前缀 `ENCv1:`** | 给将来换算法 / 换格式留升级位，也让"是不是密文"一眼可判 |

加密的核心逻辑：

```go
block, _ := aes.NewCipher(key)                 // key 固定 32 字节（AES-256）
gcm, _ := cipher.NewGCM(block)
nonce := make([]byte, 12)
rand.Read(nonce)                               // ⭐ 每次加密都重新随机
ct := gcm.Seal(nil, nonce, []byte(plain), nil)
out := "ENCv1:" + base64.StdEncoding.EncodeToString(append(nonce, ct...))
```

### 9.4 密钥管理：KEK 绝不进配置中心

| 规则 | 说明 |
|---|---|
| **主密钥来源（按优先级）** | `APP_CONFIG_SECRET_FILE`（密钥文件，推荐 —— Docker / K8s secret 挂载）**优先于** `APP_CONFIG_SECRET`（环境变量） |
| **密钥格式** | 支持 base64（44 字符）/ hex（64 字符）/ 原文 32 字节，三种都归一化成 **32 字节** |
| **绝不进 Nacos / git** | 配置中心里只有密文 —— **拖库拿到配置也无法还原** |
| **轮换要双密钥** | `APP_CONFIG_SECRET_PREV` **只参与解密**，加密**恒用 current**：轮换期新旧密文都能读，避免"换密钥导致全量配置失效" |
| **缺失时的行为** | 未配密钥 → dev 明文可用；但**一遇到 `ENCv1:` 密文就 fail-fast** —— 快速失败远好过带着空凭证启动 |

### 9.5 解密发生在哪一步

```go
// 1) 从 Nacos 拉取配置（此时敏感字段是 ENCv1 密文）
// 2) 反序列化进结构体
cipher, err := config.LoadCipher()
if err != nil {
	return err
}
// 3) 反射递归解密：遍历结构体（含嵌套、指针、切片），只处理 ENCv1: 前缀的字符串
if err := config.DecryptSecrets(cipher, &cfg); err != nil {
	return err
}
// 4) 明文只存在于内存：日志与响应里永不出现
```

| 设计点 | 为什么这么做 |
|---|---|
| **反射递归解密** | 不给每个字段写代码 —— 结构体增删字段不用改解密逻辑 |
| **就地修改** | 不额外复制一份"明文配置"到处传，缩小泄露面 |
| **解密失败即启动失败** | 密钥不匹配 / 密文被篡改 → 直接退出，不带着可疑配置继续跑 |
...
| `configctl encrypt -v '明文'` | 输出 `ENCv1:` 密文（也支持管道输入） |
| `configctl decrypt -v 'ENCv1:...'` | 排障用 —— **只在本地 CLI 用，绝不暴露成线上接口** |
| `configctl encrypt-file -f x.yaml` | 按敏感字段清单**批量**加密模板（保留注释与缩进） |
| `configctl check -f x.yaml` | 检查模板里是否残留明文敏感值 —— **可挂进 CI，阻断明文推送到 Nacos** |

> ⭐ 三个容易被忽略的设计：
> 1. **`check` 分高危 / 低危两级**：高危（password / secret / token / ak / sk 等）**直接阻断**，低危（如 username）只提示 —— 因为"什么都要求加密"最后会被团队绕过；
> 2. **调试路由只提供加密、不提供解密**：避免线上留一个"读取配置明文"的后门；要解密只能在有密钥的机器上用 CLI；
> 3. **`encrypt-file` 保留格式**：只替换值、保留缩进与行内注释，否则模板没法用 diff 评审。

### 面试官会追问什么

- **为什么用 AES-256-GCM 而不是 AES-CBC？** → GCM 是 **AEAD**，自带认证标签，能同时保证**机密性与完整性**；CBC 要额外做 HMAC 才能防篡改，否则存在 padding oracle 类攻击面。
- **密钥怎么轮换？** → 双密钥并存：新密钥作 current 负责加密，旧密钥放 previous 只负责解密；等所有配置重新加密完再移除 previous。**关键是加密永远只用 current**，否则轮换期会产生两份密钥加密的密文。
- **为什么不把密钥也放在 Nacos 里？** → 那等于**钥匙和保险箱放在一起**，拖库即全泄。密钥必须走 K8s Secret / 环境变量 / 密钥管理服务。
- **本地开发怎么办？** → dev 模板**刻意保留明文**（`check --warn-only` 不阻断），生产模板必须全密文 —— 用环境区分，而不是让开发者手工加密调试。
- **加密后的配置还能热更新吗？** → 能。热更新拿到的仍可能是 `ENCv1:` 密文，**需要在监听回调里同样走一次解密**，否则热更新会把明文替换成密文（这是最容易漏的一处）。
