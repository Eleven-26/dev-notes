...
>
> 内容整理自个人学习笔记。加密算法与数字证书的基础概念见 [../security/数字证书与PKI.md](../security/数字证书与PKI.md)、[../security/加密算法.md](../security/加密算法.md)。
...
握手的前提是 **TCP 三次握手已完成**（见 [TCP三次握手.md](./TCP三次握手.md)）——TLS 的 RTT 是**叠加在 TCP 之上**的额外开销，这也是后面为什么要压缩握手往返。另外，HTTPS 默认**只认证服务器**（单向认证），要认证客户端必须上 **mTLS**（第八、九节）。
...
### 3.4 证书是怎么验证的
证书概念见 [../security/数字证书与PKI.md](../security/数字证书与PKI.md)，这里只讲握手现场客户端做的事：
