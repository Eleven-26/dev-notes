...
>
> 内容整理自个人学习笔记；本轮新增的 inspect 取字段、prune 作用范围对照、exec/run、镜像膨胀排查、context、Compose、buildx 等小节，命令语义参考《Docker 技术入门与实战》（第 3 版，杨保华 / 戴王剑 / 曹亚仑）。本文件只收录**命令**，原理与实现见 [容器原理.md](容器原理.md)、[镜像构建与缓存.md](镜像构建与缓存.md)、[网络与存储.md](网络与存储.md)、[资源限制与运维.md](资源限制与运维.md)，CI/CD 见 [CI-CD.md](../CI-CD.md)。
...
| `docker image prune -a` | **所有没被任何容器（含已停止容器）引用的镜像** | ⚠️ 容器被 `rm` 后基础镜像就成"无主"，Runner 上会被删掉 → 构建变慢/失败（见 [CI-CD.md 第五节](../CI-CD.md)） |
...
> tag 用 commit SHA 而不是 `latest`（原因见 [CI-CD.md 第二节](../CI-CD.md)）。
...
- [../CI-CD.md](../CI-CD.md) — 这些命令在流水线里的位置
