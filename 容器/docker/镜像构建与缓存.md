...
**考察意图**：考**「构建的正规姿势」**：多阶段构建、基础镜像选择、以及为什么不在最终镜像里留编译器。面试官在意的是你有没有意识到——**每一层留下的东西都会跟着镜像走**。
...
**考察意图**：考**「有没有真被缓存坑过」**。缓存 key = 指令文本 + 基础镜像层 ID + （`COPY`/`ADD` 时）被拷文件校验和；把 `COPY . .` 放在前面会让每次提交缓存全废——这是最高频的实操错误。
