...
>
> 内容整理自大厂 Go 后端面试真题视频；本轮补充（构建缓存、ENTRYPOINT 与 CMD）参考《Docker 技术入门与实战》（第 3 版，杨保华 / 戴王剑 / 曹亚仑）。参考资料与原始素材见 [素材清单](../../素材清单.md)。

---

## 一、Docker 怎样构建镜像（build）？
...
## 二、构建：缓存失效条件、层顺序、ENTRYPOINT vs CMD ⭐
...
正确写法与失败条件见 [镜像瘦身与构建缓存.md](镜像瘦身与构建缓存.md) 的第二节）。配套习惯：系统依赖放最前、
...
| K8s 读不读 Dockerfile 的 HEALTHCHECK | — | **不读**，必须在 Pod 里写（见 [K8s部署与生命周期.md](../k8s/K8s部署与生命周期.md) 的第三节） |
