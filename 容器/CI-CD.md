...
>
> 内容整理自大厂 Go 后端面试真题视频，参考资料与原始素材见 [素材清单](../面试/素材清单.md)；本轮补充（镜像 tag 与缓存策略、Runner executor 选型、声明式部署与回滚、流水线事故）参考《Docker 技术入门与实战》（第 3 版，杨保华 / 戴王剑 / 曹亚仑）。
...
## Q2. 流水线里"构建镜像"这一段怎么做才对？

**来源**：本轮补充 · 参考《Docker 技术入门与实战》（第 3 版）
**考察意图**：考**「CI 上构建和本地构建差在哪」**。三点答不全就等于没在 CI 上真搭过：context 的传输量、`.dockerignore` 的作用、以及「Runner 每次是新容器 → **本地层缓存天然是空的**」。

**考察意图**：Q1 讲链路，这题讲**工程细节**——同样一句 `docker build`，在 CI 里要考虑上下文、缓存、tag 策略。
...
## Q3. Runner 的架构：executor 选型、并发与隔离 ⭐
...
| **docker** | 每个 job 起一个**容器**（`image:` 指定） | 环境声明式、干净、可复现 | 要宿主 daemon；构建镜像走 DooD（Q1）或 DinD |
...
| 工作目录 | 同一克隆目录不能并发复用，否则互踩（见 Q5） |
| job 资源限额 | 给 job 容器设 CPU/内存上限，避免一个 job OOM 拖垮整机其他 job |

### 三、DooD 的代价（机制见 Q1，这里只补风险）
...
② K8s 里可 `rollout undo` 走 ReplicaSet 历史（见 [K8s部署与生命周期.md](k8s/K8s部署与生命周期.md) 的 Q4）；
③ ⭐ **回滚只回滚代码，不回滚数据**——所以要求"迁移向后兼容"（先加列后改逻辑，旧版本读得懂新 schema），
否则回滚 = 故障延长。**面试官一定会追"那你们怎么保证兼容"**，这句要主动说出来。

---

## Q5. 流水线上的常见事故（比"会配 YAML"更值钱）⭐
...
| 部署"成功"但线上还是旧版本 | tag 可变（覆盖推 `latest`）+ 节点 `imagePullPolicy` 认为不用重拉 | 用 SHA tag（见本文 Q2）；K8s 侧的坑见 [K8s部署与生命周期.md](k8s/K8s部署与生命周期.md) 的 Q2 |
...
- **"CI 上为什么比本地慢？"** → 上下文大且无 `.dockerignore`、缓存不复用（每次新容器）、
  镜像源/依赖源没加速（与宿主机无关，见 K8s 篇 Q2）、Runner 并发超订把磁盘 IO 打满。
