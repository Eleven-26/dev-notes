...
**考察意图**：考**「CI 上构建和本地构建差在哪」**。三点答不全就等于没在 CI 上真搭过：context 的传输量、`.dockerignore` 的作用、以及「Runner 每次是新容器 → **本地层缓存天然是空的**」。Q1 讲链路，这题讲**工程细节**——同样一句 `docker build`，在 CI 里要考虑上下文、缓存、tag 策略。
