...
>
> 内容整理自大厂 Go 后端面试真题视频，参考资料与原始素材见 [素材清单](../interview/素材清单.md)；本轮补充（镜像 tag 与缓存策略、Runner executor 选型、声明式部署与回滚、流水线事故）参考《Docker 技术入门与实战》（第 3 版，杨保华 / 戴王剑 / 曹亚仑）。
...
| 事故 | 为什么会这样 | 怎么防 |
|---|---|---|
| ⚠️ Runner 上定时 `docker system prune -a`，之后**构建突然巨慢/失败** | 容器被 `rm` 后它用的基础镜像就成了"无主"，`-a` 把它们连同 `cache-from` 的旧层一起当垃圾删了 | 只清悬空/按时间加条件，或干脆只 `builder prune`；范围对照见 [命令速查](命令速查.md) 第八节 |
