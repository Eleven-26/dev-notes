...
>
> 内容整理自大厂 Go 后端面试真题视频；本轮补充（网络与存储模型）参考《Docker 技术入门与实战》（第 3 版，杨保华 / 戴王剑 / 曹亚仑）。参考资料与原始素材见 [素材清单](../interview/素材清单.md)。
...
- [../network/IO多路复用.md](../network/IO多路复用.md) — 端口与 fd 上限在容器内的体现
- [../linux/文件系统与IO.md](../linux/文件系统与IO.md) — Page Cache 与容器可写层的落盘
