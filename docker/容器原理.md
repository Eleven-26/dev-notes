...
>
> 内容整理自大厂 Go 后端面试真题视频；本轮补充（namespace 与 cgroup、PID 1 信号）参考《Docker 技术入门与实战》（第 3 版，杨保华 / 戴王剑 / 曹亚仑）。参考资料与原始素材见 [素材清单](../interview/素材清单.md)。
...
- [../linux/进程与线程.md](../linux/进程与线程.md) — 命名空间与进程模型的内核侧基础
- [K8s部署与生命周期.md](K8s部署与生命周期.md) — 探针与 SIGTERM 排空的运行时前提
