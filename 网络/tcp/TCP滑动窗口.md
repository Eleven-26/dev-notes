...
> 连接池、指标打点、优雅关闭（对应 [K8s部署与生命周期.md](../../容器/k8s/K8s部署与生命周期.md) 的优雅停机）都要接上。
...
| 观察点 | 机制 | 对应正文 |
|---|---|---|
| 吞吐随 RTT 上升而下降 | 带宽时延积固定，窗口决定在途数据量 | 第一节「窗口决定了能发多少」 |
| 少量丢包时不是等超时，而是 **3 个重复 ACK 触发快速重传** | SACK + 重复 ACK | 第一节追问「累积确认 → 快速重传」 |
