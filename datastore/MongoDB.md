...
判据：如果一次业务操作**总**要跨 N 个文档开事务，先怀疑建模——不变量一般应该收在同一个文档里（单文档写天然原子）。事务是兜底，不是默认路径；真有大量跨表强一致需求，说明这数据本质是关系型的（对照 [mysql/事务与隔离级别.md](mysql/事务与隔离级别.md)）。
...
- 索引体积比想象中小/大有原因：WiredTiger 对索引做前缀压缩，重复前缀越多越省；高基数随机键（完整 ObjectId、UUID）几乎压不动。机制见 [压缩算法.md](../algorithms/压缩算法.md)。
