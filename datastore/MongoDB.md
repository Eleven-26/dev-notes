...
判据：如果一次业务操作**总**要跨 N 个文档开事务，先怀疑建模——不变量一般应该收在同一个文档里（单文档写天然原子）。事务是兜底，不是默认路径；真有大量跨表强一致需求，说明这数据本质是关系型的（对照 [mysql/事务与日志.md](mysql/事务与日志.md)）。
...
对照：MongoDB 多文档事务只解决"本库内 ACID"，跨服务仍需消息/Saga 的最终一致方案 → 见 [分布式事务.md](../distributed/分布式事务.md)。
...
- 索引体积比想象中小/大有原因：WiredTiger 对索引做前缀压缩，重复前缀越多越省；高基数随机键（完整 ObjectId、UUID）几乎压不动。机制见 [压缩与淘汰算法.md](../algorithms/压缩与淘汰算法.md)。
...
| 把 TTL 当准点定时器 | 后台周期扫描 + 删除也要复制，不保证准点 | 需要准点触达用延迟消息（见 [Kafka.md](../middleware/Kafka.md)） |
