...
> ⚠️ **只编译、没运行**——本机没有 MySQL 实例，所以 SQL 会不会真按预期走索引，仍以文首那几条 `EXPLAIN` 结论为准，编译只能保证 API 与语法没错。

**跑法**（`gen.js` 的作用：把本节代码块原样贴进 `verify` 包，只在外面补 `package`/`import` 和 `Article`/`Cursor`/`UserBrief` 三个 record）：

```bash
# java 不在 PATH 里，先让 mvn 用 IDEA 自带的 JBR
export JAVA_HOME="D:\\Program Files\\IntelliJ IDEA 2023.3.2\\jbr"

cd "$TEMP/verify-maven"                                    # pom.xml + gen.js 放这儿
cp "/d/www/dev-notes/datastore/mysql/索引与优化.md" doc-mysql-index.md   # 拷成 ASCII 文件名，原因见下
node gen.js doc-mysql-index.md src/main/java/verify mysql-index.json
"/d/Program Files/apache-maven-3.8.1/bin/mvn.cmd" -B clean compile
```
...
- [查询链路.md](查询链路.md) — 优化器选路径之后的执行环节
- [JOIN与反范式.md](JOIN与反范式.md) — 关联字段上的索引设计
- [软件架构.md](软件架构.md) — 读写分离与报表查询分流
- [索引设计.md](索引设计.md) — B+Tree 与页分裂、选择性、三星索引、EXPLAIN 逐列解读
- [查询优化.md](查询优化.md) — 消除 filesort 与临时表、查询重写、COUNT 与排序优化
- [分表与路由.md](分表与路由.md) — 单表优化到头之后的分片
