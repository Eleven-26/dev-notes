...
## 四、怎样排查慢查询？（`mysqldumpslow` 实操）

**来源**：`BV12qjA6aErF p=76` 百度 Go 开发日常实习一面 · 时长 19分55秒
**考察意图**：第一节讲的是"优化思路"，本题讲的是**手上的具体动作**——
...
第一节的 B1 与第三节的索引优化都指向同一条：**`SELECT *` 会毁掉覆盖索引**。同一句话，只改 select 列，执行计划就从 `Using index condition` 变成 `Using index`：
