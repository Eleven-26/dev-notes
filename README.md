...
| [设计模式/README.md](php/设计模式/README.md) | 本目录索引：GoF 分类表、模式边界速查表、阅读顺序、校验口径 |
| [设计模式/六大原则.md](php/设计模式/六大原则.md) | SRP/OCP/LSP/ISP/DIP + 迪米特 + 合成复用，每条配一个可跑的反例；原则→模式对照表 |
| [设计模式/单例模式.md](php/设计模式/单例模式.md) | 私有构造/`clone`/`unserialize` 三条约束、php-fpm 下单例只活一次请求、容器 `singleton` 写法 |
| [设计模式/工厂模式.md](php/设计模式/工厂模式.md) | 简单工厂/工厂方法/抽象工厂在 PHP 里的真实形态、`new self()` vs `new static()`、工厂与容器 DI 的边界 |
| [设计模式/原型模式.md](php/设计模式/原型模式.md) | `clone` 浅拷贝的不对称语义、深拷贝三种做法与代价、`__clone` 里处理静态属性与资源句柄 |
| [设计模式/注册模式.md](php/设计模式/注册模式.md) | 全局对象表 vs 单例 vs 容器、静态注册表跨请求不存活与常驻串号、`ArrayAccess` 写法 |
| [设计模式/适配器模式.md](php/设计模式/适配器模式.md) | 对象适配器、类适配器在 PHP 里排不上号（`trait` 是假继承）、`__call` 万能适配器的代价 |
| [设计模式/装饰器模式.md](php/设计模式/装饰器模式.md) | 标准形状与两个冒牌货、装饰顺序会改变结果、`__call` 转发、PSR-7 中间件栈 |
| [设计模式/代理模式.md](php/设计模式/代理模式.md) | 远程/虚拟/缓存/保护四类代理、"没访问就不构造"的实测、PHP 无 JDK 动态代理 |
| [设计模式/外观模式.md](php/设计模式/外观模式.md) | 外观 vs 中介者、上帝服务判据、Controller→Service→Repository 收敛成用例方法 |
| [设计模式/组合模式.md](php/设计模式/组合模式.md) | Part-Whole 树、`RecursiveIterator` 先序/后序、透明性 vs 安全性、成环与防环 |
| [设计模式/过滤器模式.md](php/设计模式/过滤器模式.md) | 谓词一等公民、`andX/orX/notX` 组合、与 SQL 下推和 Laravel Scope 的取舍 |
| [设计模式/观察者模式.md](php/设计模式/观察者模式.md) | `SplObjectStorage` 实测、通知隔离与快照、观察者表随请求销毁、跨请求只能落队列 |
| [设计模式/策略模式.md](php/设计模式/策略模式.md) | 接口/闭包/`match` 三种写法的判据、策略带状态时的复用问题、什么时候别上策略 |
| [设计模式/迭代器模式.md](php/设计模式/迭代器模式.md) | `foreach` 对数组与对象的语义差异、`Iterator` vs `IteratorAggregate` vs `yield`、生成器二次遍历报错 |
| [设计模式/面向切面编程.md](php/设计模式/面向切面编程.md) | 纯 PHP 四种切面（装饰器/闭包管道/`__call`/`#[Attribute]`+反射）、与真正织入的差距、何时不该用 |
