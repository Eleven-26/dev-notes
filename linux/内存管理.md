...
1. **虚拟内存有什么用？** → 隔离（每进程独立地址空间）、简化链接与加载、**超售 + 延迟分配**（malloc 不立刻拿物理页）、支持内存映射文件与共享内存。核心是**"让进程以为自己独占一整块连续内存"**（见一）。
2. **页表与 TLB 是什么？为什么需要 TLB？** → 页表是 VA→PA 的多级映射表（x86-64 4~5 级）；TLB 是**页表的高速缓存**。多级页表意味着一次转换要多次访存，TLB 命中则一次完成，⭐ **TLB miss 是进程切换的隐性成本**，大页可减少 TLB 条目数（见一）。
3. **缺页中断有哪两种？** → **minor fault**：页已在物理内存、只是没建映射（首次写 malloc 的内存、COW），不读盘；**major fault**：页不在内存，须从磁盘/swap 读入，代价高 3 个数量级（见一）。
4. **伙伴系统和 Slab 分别解决什么问题？** → 伙伴系统按 2^order 连续页管理、分裂与合并，**解决外部碎片**；Slab/Slub 把页切成同规格对象池，**解决内部碎片并加速小对象分配**。类比：Buddy 批发整页，Slab 零售小件（见一）。
5. ⭐ **buffer 和 cache 有什么区别？** → 历史上 Cache 缓存**文件内容**（以页为单位）、Buffer 缓存**块设备裸块**；⚠️ **现代 Linux 两者已统一**，Buffer Cache 并入 Page Cache，`free` 里 `buff` 主要体现块设备元数据缓存（见二）。
6. ⭐ **为什么 `free` 显示内存用完了但系统不卡？** → 空闲内存被内核拿来做了 **Page Cache**，⚠️ **这部分随时可回收**。要看 `available` 而非 `free`；只有 `available` 很小、swap 持续增长、`si/so` 长期非 0 才是真紧张（见二）。
