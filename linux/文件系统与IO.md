...
7. **inode 是什么、存什么？硬链接和软链接的区别？** → inode 存文件元数据（类型、权限、大小、**链接数**、时间戳、**数据块指针**），⚠️ 不存文件名与内容；硬链接**同 inode**、不能跨 fs、删源文件后仍可用、不能对目录创建，软链接是**独立 inode**、可跨 fs、源文件删除后失效、可对目录创建（见一）。
8. ⭐ **`write()` 返回成功数据就落盘了吗？** → ❌ 没有。`write` 只把数据拷进**页缓存**就返回，页被标脏后由内核异步回写。必须调用 **`fsync` / `fdatasync`**（mmap 用 `msync`）并等其返回 0，才保证断电不丢（见二）。
