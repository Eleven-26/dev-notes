...
- [JVM与垃圾回收.md](../java/JVM与垃圾回收.md)：三色标记在 JVM 的落地、收集器组合、安全点与调优命令、类加载对根扫描的影响见 [类加载机制.md](../java/类加载机制.md)。
- [内存分配器.md](../go/运行时/内存分配器.md)：Go 的 mcache/mcentral/sizeclass 分层与"无整理靠分配器缓解碎片"的实现。
- [内存逃逸.md](../go/运行时/内存逃逸.md)：分配速率问题的代码侧根源（逃逸决定堆分配量，直接影响 GC 压力）。
- [复杂度分析.md](../算法/复杂度分析.md)：本文反复用到的"阶段复杂度 × 堆大小/存活集大小"分析方法。
